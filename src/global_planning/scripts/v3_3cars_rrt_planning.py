...
if __name__ == '__main__':
    ...
    init_car3pos = gazebo2pgm(600,30,[float(confs.config_get('PLANNING','car3_x')),float(confs.config_get('PLANNING','car3_y'))])
    ...
    if flag1 & flag2:
        while True:
            ...
            for i in range(0,len(path)):
                for j in range(-int(l_2),int(l_2)+1):
        ...
        for gostep in range(1,len(path)):
            ...
            dest_angle = angles[gostep]
            pose1 = car1.pose_get()
            pose2 = car2.pose_get()
            pose3 = car3.pose_get()
            cur_car1 = gazebo2pgm(600,30,[pose1.position.x, pose1.position.y])
            cur_car2 = gazebo2pgm(600,30,[pose2.position.x, pose2.position.y])
            cur_car3 = gazebo2pgm(600,30,[pose3.position.x, pose3.position.y])

            cur_pos = (cur_car1+cur_car2)/2
            cur_theta = math.atan2((cur_car2[1]-cur_car1[1]), (cur_car2[0]-cur_car1[0]))      # -pi 到 pi      

            dest_1 = destination - l_2*np.array([math.cos(angles[gostep]),math.sin(angles[gostep])])
            dest_2 = destination + l_2*np.array([math.cos(angles[gostep]),math.sin(angles[gostep])])
            dest_3 = destination

            t = max(norm(dest_1,cur_car1) / vel_max, norm(dest_2,cur_car2) / vel_max)
            v1 = norm(dest_1,cur_car1) / t
            v2 = norm(dest_2,cur_car2) / t
            v3 = norm(dest_3,cur_car3) / t
            ...
            fi_3 = worldeu_2_mapeu(math.atan2(-(dest_3[1]-cur_car3[1]),dest_3[0]-cur_car3[0]))
            ...
            fi3_cur = quat2eu(pose3.orientation.z, pose3.orientation.w)
            ...
            while ((abs(fi1_cur - fi_1) > 0.02) &  (abs(fi1_cur - fi_1) < math.pi*2-0.02)) | ((abs(fi2_cur - fi_2)> 0.02 ) & (abs(fi2_cur - fi_2) < math.pi*2-0.02)):
                ...
                if ((fi3_cur - fi_3 > 0.02) & (fi3_cur - fi_3 < math.pi)) | ((fi3_cur - fi_3 < -math.pi+0.02)) :
                    yaw3=yaw_max
                elif ((fi3_cur - fi_3 < -0.02)& (fi3_cur - fi_3 > -math.pi)) | ((fi3_cur - fi_3 > math.pi-0.02)) :
                    yaw3=-yaw_max


                car1.husky_cmd(0, yaw1)
                car2.husky_cmd(0, yaw2)
                car3.husky_cmd(0, yaw3)
            ...
            while norm(destination,cur_pos) > 10 :
                ...
                v2_t = 0.0
                v3_t = 0.0
                if norm(dest_1,cur_car1) > 10:
                    v1_t = v1
                if norm(dest_2,cur_car2) > 10:
                    v2_t = v2
                if norm(dest_3,cur_car3) > 10:
                    v3_t = v3
                print(dest_1,cur_car1,'--',dest_2,cur_car2,'--',dest_3,cur_car3)
                car1.husky_cmd(v1_t, 0)
                car2.husky_cmd(v2_t, 0)
                car3.husky_cmd(v3_t, 0)
                rospy.sleep(duration)

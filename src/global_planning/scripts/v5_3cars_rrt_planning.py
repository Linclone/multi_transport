...
class PurePursuit:
    def __init__(self, path):
        self.path = path
        self.lookahead_distance = 0.7  # 预瞄距离
        self.current_idx = 0
    ...
    def search_target(self, pose):
        ...
        for i in range(nearest_idx, len(self.path)):
            if np.hypot(self.path[i][0]-pose.position.x, self.path[i][1]-pose.position.y) >= self.lookahead_distance:
                self.current_idx = i
                return nearest_idx, self.path[i]
        return nearest_idx, self.path[-1]
...
class Base_confi():
    ...
    def __init__(self):
        self.configfile_path = os.path.join(os.path.dirname(__file__), '..', 'config/3cars_config.ini')
...
if __name__ == '__main__':
    ...
    init_car3pos = np.array([float(confs.config_get('PLANNING','car3_x')),float(confs.config_get('PLANNING','car3_y'))])
    ...
    if flag1 & flag2:
        while True:
            ...
            for i in range(1,len(path0)):
                patht=RRT_star(map,path0[i-1],path0[i],40,0.7,15,rigid_size,scan_rate)
        ...
        for gostep in range(1,len(path)):
            ...
            PATH1_t, PATH2_t = path_gen(pgm2gazebo(600, 30, path[gostep-1].astype(int)), pgm2gazebo(600, 30, path[gostep].astype(int)), mapeu_2_worldeu(angles[gostep-1]), mapeu_2_worldeu(angles[gostep]), 32, 2*l_2)
            PATH3_t, _= path_gen(pgm2gazebo(600, 30, path[gostep-1].astype(int)), pgm2gazebo(600, 30, path[gostep].astype(int)), mapeu_2_worldeu(angles[gostep-1]), mapeu_2_worldeu(angles[gostep]), 32, 0)
        ...
        for c in range(0,len(PATH1)):
            point=gazebo2pgm(600, 30, PATH1[c])
            map[int(round(point[1])),int(round(point[0]))]=0
        ...
        cur_car3 = np.array([pose3.position.x, pose3.position.y])
        ...
        while(True):
            ...
            if norm(PATH1[-1], cur_car1)>0.5:
                nearest_idx1, target1 = controller1.search_target(car1.pose_get())

                if nearest_idx1<len(PATH1)-3:
                    dest1=norm(PATH1[nearest_idx1], PATH1[nearest_idx1+3]) * 10
                else:
                    dest1=norm(PATH1[nearest_idx1], PATH1[-1]) * 10


            if norm(PATH2[-1], cur_car2)>0.5:
                nearest_idx2, target2 = controller2.search_target(car2.pose_get())
                
                if nearest_idx2<len(PATH2)-3:
                    dest2=norm(PATH2[nearest_idx2], PATH2[nearest_idx2+3]) * 10
                else:
                    dest2=norm(PATH2[nearest_idx2], PATH1[-1]) * 10

            if norm(PATH3[-1], cur_car3)>0.5:
                nearest_idx3, target3 = controller3.search_target(car3.pose_get())
                
                if nearest_idx3<len(PATH3)-3:
                    dest3=norm(PATH3[nearest_idx3], PATH3[nearest_idx3+3]) * 10
                else:
                    dest3=norm(PATH3[nearest_idx3], PATH1[-1]) * 10

            if (dest1==0) & (dest2==0) & (dest3==0):
                break

            maximun = max(dest1, dest2, dest3)
            v1_out = vel_max * dest1/maximun
            v2_out = vel_max * dest2/maximun
            v3_out = vel_max * dest3/maximun
            omega1 = pure_pursuit_control(car1.pose_get(), target1, v1_out)
            omega2 = pure_pursuit_control(car2.pose_get(), target2, v2_out)
            omega3 = pure_pursuit_control(car3.pose_get(), target3, v3_out)

            print(nearest_idx2, ":  ", dest1, dest2, dest3, "---", v1_out, v2_out, v3_out)


            car1.husky_cmd(v1_out, omega1)
            car2.husky_cmd(v2_out, omega2)
            car3.husky_cmd(v3_out, omega3)

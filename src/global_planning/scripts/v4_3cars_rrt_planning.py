...
if __name__ == '__main__':
    ...
    if flag1 & flag2:
        ...
        for gostep in range(1,len(path)):
            ...
            fi3_cur = worldeu_2_mapeu(quat2eu(pose3.orientation.z, pose3.orientation.w))

            PATH1, PATH2 = path_gen(cur_pos, destination, cur_theta, dest_angle, 7, 2*l_2)
            PATH3, _= path_gen(cur_pos, destination, cur_theta, dest_angle, 7, 0)
            ...
            for cc in range(1,len(PATH1)):
                ...
                dis3 += norm(PATH3[cc-1],PATH3[cc])

            print(dis1, dis2, dis3)
            maximun = max(dis1, dis2, dis3)
            ...
            while norm(destination,cur_pos) > 0.5 :
                ...
                if norm(dest_3, cur_car3)>0.5:
                    target3 = controller3.search_target(car3.pose_get())
                    v3_out = vel_max * dis3/maximun

                if (v1_out==0) & (v2_out==0) & (v3_out==0):
                    break

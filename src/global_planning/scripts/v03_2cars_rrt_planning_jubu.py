...
class PurePursuit:
    def __init__(self, path):
        self.path = path
        self.lookahead_distance = 0.25  # 预瞄距离
        self.current_idx = 0
...
def RRT_star(map: np.array, S: np.array, E: np.array, step, pro, R, rigid_size,scan_rate):
    ...
    while distance > step:
        ...
        if random.random() < pro:
            ...
            P_rand = np.array([random.random()*(xmax-xmin)+xmin,random.random()*(ymax-ymin)+ymin])
...
if __name__ == '__main__':
    ...
    rigid_size=(rigid_size*600/9).astype(int)
    ...
    print("car1 pose = ",cur_car1,"; car2 pose =",cur_car2)

    S=np.array(gazebo2pgm(600,9,[float(confs.config_get('RIGID','Sx')),float(confs.config_get('RIGID','Sy'))])).astype(int)
    # S_real=0.5*(cur_car1+cur_car2)
    # S=np.array(gazebo2pgm(600,9,[S_real[0],S_real[1]])).astype(int)
    E=np.array(gazebo2pgm(600,9,[float(confs.config_get('PLANNING','Ex')),float(confs.config_get('PLANNING','Ey'))])).astype(int)
    print('起点：', S)
    print('终点：', E)
    ...
    if flag1 & flag2:
        ...
        costmin=9999
        for j in range(0,1):
            path0t = RRT_star(map,S,E,40,0.5,100,rigid_size,scan_rate)
            print('RRT_rough Finished! -- ',j+1,'/1')
            cost_t=0.0
        ...
        for i in range(1,len(path0)):
            patht=RRT_star(map,path0[i-1],path0[i],20,0.6,55,rigid_size,scan_rate)
        ...
        print(cur_car1,cur_car2)

        for i in range(0,len(path)):
            for j in range(-int(600/9*l_2),int(600/9*l_2)+1):
        ...
        for gostep in range(1,len(path)):
            ...
            destination = pgm2gazebo(600, 9, path[gostep])
            ...
            sampling_rate = 20
            PATH1, PATH2 = path_gen(cur_pos, destination, cur_theta, dest_angle, sampling_rate, 2*l_2)
            dest_1 = PATH1[-1]
            dest_2 = PATH2[-1]

            next_1 = PATH1[round(sampling_rate/5)]
            next_2 = PATH2[round(sampling_rate/5)]

            fi_1 = worldeu_2_mapeu(math.atan2((next_1[1]-cur_car1[1]),next_1[0]-cur_car1[0]))
            fi_2 = worldeu_2_mapeu(math.atan2((next_2[1]-cur_car2[1]),next_2[0]-cur_car2[0]))

            # print(fi_1,fi_2,'====',dest_1,dest_2)
            pose1 = car1.pose_get()
            pose2 = car2.pose_get()
            cur_car1 = np.array([pose1.position.x, pose1.position.y])
            cur_car2 = np.array([pose2.position.x, pose2.position.y])

            print("car1: {",dest_1, cur_car1,"}")
            print("car1: {",dest_2, cur_car2,"}")
            print(norm(dest_1, cur_car1))

            while ((abs(fi1_cur - fi_1) > 0.1) &  (abs(fi1_cur - fi_1) < math.pi*2-0.1)) | ((abs(fi2_cur - fi_2)> 0.1 ) & (abs(fi2_cur - fi_2) < math.pi*2-0.1)):
                yaw1 = 0.0
                yaw2 = 0.0
                if ((fi1_cur - fi_1 > 0.1) & (fi1_cur - fi_1 < math.pi)) | ((fi1_cur - fi_1 < -math.pi+0.1)) :
                    yaw1=yaw_max
                elif ((fi1_cur - fi_1 < -0.1)& (fi1_cur - fi_1 > -math.pi)) | ((fi1_cur - fi_1 > math.pi-0.1)) :
                    yaw1=-yaw_max

                if ((fi2_cur - fi_2 > 0.1) & (fi2_cur - fi_2 < math.pi)) | ((fi2_cur - fi_2 < -math.pi+0.1)) :
                    yaw2=yaw_max
                elif ((fi2_cur - fi_2 < -0.1)& (fi2_cur - fi_2 > -math.pi)) | ((fi2_cur - fi_2 > math.pi-0.1)) :
                    yaw2=-yaw_max
            ...
            while norm(destination,cur_pos) > 0.15 :
                v1_out=0
                v2_out=0
                omega1=0
                omega2=0

                print(norm(dest_1, cur_car1),'---',norm(dest_2, cur_car2))

                if norm(dest_1, cur_car1)>0.1:
                    target1 = controller1.search_target(car1.pose_get())
                    v1_out = vel_max * dis1/maximun

                if norm(dest_2, cur_car2)>0.1:
